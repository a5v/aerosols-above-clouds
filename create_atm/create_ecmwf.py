...
if __name__ == '__main__':
    ...
    #dTau = np.insert(dTau, 0, atmfunc.RdTau(1, df_new['*PRE'][0], df_new['*PRE'][50]))
    #dTau = np.insert(dTau, 1, atmfunc.RdTau(1, df_new['*PRE'][1], df_new['*PRE'][2]))

    dTau = np.insert(dTau, 0, 1) 
    ...
    # with open('./reformat_ecmwf_atm/ecmwf_rc01.csv', 'w') as file:
    ...
    [rfldir, rfldn, flup, dfdt, uavg, uu, albmed, trnmed] =\
                disort.run(dTau=dTau, w0=w0, iphas=iphas, uphas = p, gg=gg,
                           umu0=umu0, phi0=phi0, albedo=albedo, fbeam=fbeam,
                           umu=umu, phi=phi, prnt=prnt)
